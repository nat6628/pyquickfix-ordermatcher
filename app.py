...
def get_data_symbol():
    ...
    data = data.rename(columns={'symbol': 'Symbol', 'close': 'Close', 'limitUp': 'Limit Up', 'limitDown': 'Limit Down', 'open': 'Open', 'high': 'High', 'low': 'Low', 'lastPrice': 'Last Price', 'volume': 'Volume'})
    numeric_cols = ['Close', 'Limit Up', 'Limit Down', 'Open', 'High', 'Low', 'Last Price', 'Volume']  
    data[numeric_cols] = data[numeric_cols].applymap(lambda x: '{:.2f}'.format(x))
    conn.close()
    return data

def get_pending_orders():
    conn = sqlite3.connect('database.db')
    data = pd.read_sql_query("SELECT clOrdID, symbol, side, ordType, price, quantity, executedQuantity, lastExecutedQuantity, lastExecutedPrice, openQuantity FROM pending_order", conn)
    data = data.rename(columns={'clOrdID': 'Order ID', 'symbol': 'Symbol', 'side': 'Side', 'ordType': 'Order Type', 'price': 'Price', 'quantity': 'Quantity', 'executedQuantity': 'Executed Quantity', 'lastExecutedQuantity': 'Last Executed Quantity', 'lastExecutedPrice': 'Last Executed Price', 'openQuantity': 'Open Quantity'})
    mumeric_cols = ['Price', 'Quantity', 'Executed Quantity', 'Last Executed Quantity', 'Last Executed Price', 'Open Quantity']
    data[mumeric_cols] = data[mumeric_cols].applymap(lambda x: '{:.2f}'.format(x))
    conn.close()
    return data

def get_order_history():
    conn = sqlite3.connect('database.db')
    data = pd.read_sql_query("SELECT clOrdID, symbol, side, ordType, price, quantity, executedQuantity, lastExecutedQuantity, lastExecutedPrice FROM order_history", conn)
    data = data.rename(columns={'clOrdID': 'Order ID', 'symbol': 'Symbol', 'side': 'Side', 'ordType': 'Order Type', 'price': 'Price', 'quantity': 'Quantity', 'executedQuantity': 'Executed Quantity', 'lastExecutedQuantity': 'Last Executed Quantity', 'lastExecutedPrice': 'Last Executed Price'})
    mumeric_cols = ['Price', 'Quantity', 'Executed Quantity', 'Last Executed Quantity', 'Last Executed Price']
    data[mumeric_cols] = data[mumeric_cols].applymap(lambda x: '{:.2f}'.format(x))
    conn.close()
    return data
...
def app(): 
    ...
    if page == "Home":
        ...
        order_page = st.sidebar.selectbox('Order Page', ['Pending Orders', 'Order History'], key='order_page')
        if order_page == "Pending Orders":
            ...
            if action == 'Modify':
                with st.sidebar.form(key='modify_form'):
                    ...
                    selected_order_id = st.selectbox('Order ID', order_ids, format_func=lambda x: x if x else 'No orders available')
                    ...
                    if st.form_submit_button('Submit'):
                        st.write(f'Order {order_id} has been modified to {new_symbol}, {new_side}, {new_type}, {new_price}, {new_quantity}')
            if action == 'Cancel':
